...
export const tableSearchValidate = (bug, searchValues) => {
    ...
    if (bug[ID].toString().includes(searchValues[ID]) &&
        ...
        (!bug[DATE_OF_CREATION] || (bug[DATE_OF_CREATION] && bug[DATE_OF_CREATION].includes(searchValues[DATE_OF_CREATION]))) &&
        (!bug[DATE_OF_CHANGE] || (bug[DATE_OF_CHANGE] && bug[DATE_OF_CHANGE].includes(searchValues[DATE_OF_CHANGE]))) &&
        (!bug[CLOSING_DATE] || (bug[CLOSING_DATE] && bug[CLOSING_DATE].includes(searchValues[CLOSING_DATE]))) &&
        (!bug[DETECTION_METHOD] || (bug[DETECTION_METHOD] && bug[DETECTION_METHOD].includes(searchValues[DETECTION_METHOD]))) &&
        (!bug[REOPENS_AMOUNT] || (bug[REOPENS_AMOUNT] && bug[REOPENS_AMOUNT].includes(searchValues[REOPENS_AMOUNT])))) {
        return true;
    }
};